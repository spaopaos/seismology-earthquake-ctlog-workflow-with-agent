...
The current maintained stage uses MESS CC and independent PhaseNet+ catalog arrivals, IDAT=3. It produces exactly three independent CC-threshold catalogs. DAMP selection uses the same spatial-evidence rule as first-round relocation, with new trials for each detection tier.
...
The matching windows, joint iteration schedule and three CC products are explicit project starting settings, not universal paper-derived optima. Independent PhaseNet arrival values do not make template-guided event selection statistically independent. Upstream ERH is only a declared comparison scale for MESS events.
